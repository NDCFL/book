...
$('#mytab').bootstrapTable({
    ...
    url: "/employee/employeeList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value) {
                if (value == 0) {
                    //表示启用状态
                    return '<span style="color: green">过客</span>';
                } else {
                    //表示启用状态
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                ...
                var g = '<a title="初始化密码" href="javascript:void(0);" onclick="initPwd(' + row.id+')"><i class="glyphicon glyphicon-phone" style="color:green">初始化密码</i></a>'
...
function del(employee, status) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/employee/deleteemployee/' + employee,
            dataType: 'json',
            success: function (data) {
...
function edit(name){
    $.post("/employee/findEmployee/"+name,
        function(data){
...
function updatestatus(id, status) {
    $.post("/employee/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/employee/employeeList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/employee/employeeList'});
}
