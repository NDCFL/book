...
$('#mytab').bootstrapTable({
    ...
    url: "/booksDiscuss/booksDiscussList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="booksDiscuss"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(booksDiscussid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/booksDiscuss/deleteBooksDiscuss/' + booksDiscussid,
            dataType: 'json',
...
function edit(name) {
    $.post("/booksDiscuss/findBooksDiscuss/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/booksDiscuss/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/booksDiscuss/booksDiscussList'});
})
function getList(val) {
    $('#mytab').bootstrapTable('refresh',
        {
            url: '/booksDiscuss/booksDiscussList',
            query:{
                "id":val
            }
...
$("#update").click(function () {
    $.post(
        "/booksDiscuss/booksDiscussUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/booksDiscuss/booksDiscussAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量修改小说类型的状态吗？', function (index) {
        $.post(
            "/booksDiscuss/deleteManyBooksDiscuss",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
