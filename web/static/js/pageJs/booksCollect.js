...
$('#mytab').bootstrapTable({
    ...
    url: "/booksCollect/booksCollectList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="booksCollect"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(booksCollectid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/booksCollect/deleteBookType/' + booksCollectid,
            dataType: 'json',
...
function edit(name) {
    $.post("/booksCollect/findBookType/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/booksCollect/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/booksCollect/booksCollectList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/booksCollect/booksCollectList'});
}
$("#update").click(function () {
    $.post(
        "/booksCollect/booksCollectUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/booksCollect/booksCollectAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量修改小说类型的状态吗？', function (index) {
        $.post(
            "/booksCollect/deleteManyBookType",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
